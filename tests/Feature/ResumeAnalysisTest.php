...
class ResumeAnalysisTest extends TestCase
{
    ...
    public function test_user_cannot_analyze_more_than_five_resumes_before_the_8am_reset(): void
    {
        config()->set('ai.resume_analyzer.daily_limit', 5);
        config()->set('ai.resume_analyzer.reset_hour', 8);
        config()->set('ai.resume_analyzer.cooldown_minutes', 0);

        $this->travelTo(Carbon::parse('2026-07-14 09:00:00', config('app.timezone')));
        $user = User::factory()->create(['onboarding_completed_at' => now()]);
        $application = $this->applicationFor($user);

        for ($count = 0; $count < 5; $count++) {
            $this->createAnalysis($user, $application);
        }

        $this->actingAs($user)
            ->withHeader('Accept', 'application/json')
            ->post(route('resume-analyses.store'), $this->analysisPayload($application))
            ->assertStatus(422)
            ->assertJsonValidationErrors('analysis');
    }

    public function test_resume_analysis_limit_resets_at_8am(): void
    {
        config()->set('ai.resume_analyzer.daily_limit', 5);
        config()->set('ai.resume_analyzer.reset_hour', 8);
        config()->set('ai.resume_analyzer.cooldown_minutes', 0);
        ...
        $this->travelTo(Carbon::parse('2026-07-14 07:59:00', config('app.timezone')));
        $user = User::factory()->create(['onboarding_completed_at' => now()]);
        $application = $this->applicationFor($user);
        ...
        $this->travelTo(Carbon::parse('2026-07-14 08:01:00', config('app.timezone')));
    ...
    public function test_user_must_wait_five_minutes_between_resume_analyses(): void
    {
        config()->set('ai.resume_analyzer.daily_limit', 5);
        config()->set('ai.resume_analyzer.reset_hour', 8);
        config()->set('ai.resume_analyzer.cooldown_minutes', 5);
        ...
        $this->travelTo(Carbon::parse('2026-07-14 09:03:00', config('app.timezone')));
    ...
    private function analysisPayload(Application $application): array
    {
        return [
            'job_source' => 'custom',
            'job_application_id' => $application->application_id,
    ...
    private function createAnalysis(User $user, Application $application): ResumeAnalysis
    {
